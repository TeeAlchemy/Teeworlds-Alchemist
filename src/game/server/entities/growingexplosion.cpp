...
void CGrowingExplosion::Tick()
{
				...
				if(FromLeft || FromRight || FromTop || FromBottom)
				{
					...
					switch(m_ExplosionEffect)
					{
						...
						case GROWINGEXPLOSIONEFFECT_HEAL_HUMANS:
							...
							break;
						case GROWINGEXPLOSIONEFFECT_LOVE:
							if(random_prob(0.2f))
							{
	...
	for(CCharacter *p = (CCharacter*) GameWorld()->FindFirst(CGameWorld::ENTTYPE_CHARACTER); p; p = (CCharacter *)p->TypeNext())
	{
		...
		if((m_pGrowingMap[k] >= 0) && !p->GetPlayer()->IsBot())
		{
			if(tick - m_pGrowingMap[k] < Server()->TickSpeed()/4)
			{
				switch(m_ExplosionEffect)
				{
					case GROWINGEXPLOSIONEFFECT_HEAL_HUMANS:
						...
						break;
					case GROWINGEXPLOSIONEFFECT_FREEZE_HUMAN:
						// p->Freeze(4.0f, m_Owner, FREEZEREASON_FLASH); TODO
		...
		if(m_pGrowingMap[k] >= 0)
		{
			if(tick - m_pGrowingMap[k] < Server()->TickSpeed()/4)
			{
				...
				{
					case GROWINGEXPLOSIONEFFECT_FREEZE:
						// p->Freeze(3.0f, m_Owner, FREEZEREASON_FLASH); TODO
						GameServer()->SendEmoticon(p->GetPlayer()->GetCID(), EMOTICON_QUESTION);
						m_Hit[p->GetPlayer()->GetCID()] = true;
						break;
					case GROWINGEXPLOSIONEFFECT_POISON:
						// p->Poison(g_Config.m_InfPoisonDamage, m_Owner); TODO
					...
					case GROWINGEXPLOSIONEFFECT_ELECTRIC:
					{
						...
						p->TakeDamage(normalize(p->GetPos() - m_SeedPos)*10.0f, Damage, m_Owner, WEAPON_HAMMER);
						m_Hit[p->GetPlayer()->GetCID()] = true;
						break;
					}
					case GROWINGEXPLOSIONEFFECT_FREEZE_HUMAN:
	...
	/* TODO
	// clean slug slime
	if (m_ExplosionEffect == GROWINGEXPLOSIONEFFECT_FREEZE) 
	{
