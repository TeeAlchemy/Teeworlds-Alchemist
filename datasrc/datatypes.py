...
class BaseType:
    def __init__(self, type_name):
        ...
        self._id = (
            GetID()
        )  # this is used to remember what order the members have in structures etc
...
class NetObject:
    ...
    def emit_declaration(self):
        ...
        if self.base:
            lines += [
                f"struct {self.struct_name} : public {self.base_struct_name}",
                "{",
            ]
        else:
    ...
    def emit_uncompressed_unpack_and_validate(self, base_item):
        ...
        lines += ["{"]
        lines += [
            f"\t{self.struct_name} *pData = ({self.struct_name} *)m_aUnpackedData;"
        ]
        unpack_lines = []
        ...
        for v in variables:
            if not self.validate_size and v.default is None:
                raise ValueError(
                    f"{v.name} in {self.name} has no default value. Member variables that do not have a default value cannot be used in a structure whose size is not validated."
                )
...
class NetStringStrict(NetVariable):
    ...
    def emit_unpack_msg(self):
        return [
            f"pData->{self.name} = pUnpacker->GetString(CUnpacker::SANITIZE_CC|CUnpacker::SKIP_START_WHITESPACES);"
        ]
...
class NetIntAny(NetVariable):
    ...
    def emit_uncompressed_unpack_obj(self):
        ...
        return [
            f"pData->{self.name} = pUnpacker->GetUncompressedIntOrDefault({self.default});"
        ]
...
class NetIntRange(NetIntAny):
    ...
    def emit_validate_obj(self):
        return [
            f'pData->{self.name} = ClampInt("{self.name}", pData->{self.name}, {self.min}, {self.max});'
        ]

    def emit_unpack_msg_check(self):
        return [
            f'if(pData->{self.name} < {self.min} || pData->{self.name} > {self.max}) {{ m_pMsgFailedOn = "{self.name}"; break; }}'
        ]
